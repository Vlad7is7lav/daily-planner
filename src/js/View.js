...
class View extends EventEmitter {
  ...
  onload() {
    this.varsObject.calendarDate_year =
      document.querySelector(".calendarDate_year")
    this.varsObject.calendarDate_month = document.querySelector(
      ".calendarDate_month"
    )
  ...
  addEventListeners() {
    ...
    formSignIn.addEventListener("submit", (event) =>
      this.loginUser(event, formSignIn.lastElementChild)
    )

    const formSignUp = document.getElementById("form-sign-up")
    formSignUp.addEventListener("submit", (event) =>
      this.registerUser(event, formSignUp.lastElementChild)
    )

    const openLoginForm = document.querySelector(".login")
    openLoginForm.addEventListener(
      "click",
      this.openLoginForm.bind(
        this,
        formSignIn.lastElementChild,
        formSignUp.lastElementChild
      )
    )
    ...
    this.varsObject.calendarDate_year.addEventListener(
      "click",
      this.openSelectYear.bind(this)
    ) // watch when click active on 'year' for choice
    this.varsObject.calendarDate_month.addEventListener(
      "click",
      this.openSelectMonth.bind(this)
    ) // watch when click active on 'month' for choice
  ...
  openLoginForm(buttonSighIn, buttonSignUp) {
    ...
    buttonSignInForm.addEventListener(
      "click",
      this.openSignInForm.bind(this, wrapper)
    )
    buttonSignUpForm.addEventListener(
      "click",
      this.openSignUpForm.bind(this, wrapper)
    )
    Array.prototype.slice.call(backToMain).forEach((elem) => {
      elem.addEventListener("click", this.backToMain.bind(this, wrapper))
    })
    if (loginForm.classList.contains("showLoginForm"))
      return this.closeLoginForm()
  ...
  showRegisterMessage(data) {
    switch (data) {
      ...
      case 0:
        ...
        elem_2.innerHTML =
          "Congratulation! </br> Use your login and pass to sign in!"
    ...
    switch (data) {
      ...
      case 0:
        ...
        setTimeout(() => {
          ...
          this.reRender() //clear tabs and tabsContent
        }, 1000)
        break
      case 2:
  ...
  //if login then show logout button in the head line
  openExitForm() {
    const login = document.querySelector(".login")
    let logout = document.createElement("div")
    logout.classList.add("login")
    logout.innerHTML =
      '<i class="fas fa-sign-out-alt" style="margin-right: 5px;"></i> Выход'
  ...
  //clear tabs and tabsContent
  reRender() {
    let currentDate = new Date()
    this.changeDate(currentDate.getFullYear(), currentDate.getMonth())
    let node = document.querySelector(".tabs")
    node.textContent = "" // remove tabs
    node.nextElementSibling.textContent = "" // remove tabs content
  }
  ...
  showPlus(event) {
    ...
    let listener = elem.addEventListener(
      "click",
      this.addNameForTab.bind(this, event)
    )
    ...
    if (data.length < 3) {
      ...
      goButton.onclick = () => {
        if (this.checkName(popupInput.value, data)) {
          ...
          setTimeout(() => {
            popupInput.value = ""
            popupInput.placeholder = "Enter the list name"
          }, 1000)
        } else {
  ...
  closeSelected(event) {
    ...
    if (target.className == "cells") {
      ...
      target.className != year.className &&
      target.className != month.className
    ) {
      this.selectYear.classList.remove("beVisible")
      this.selectMonth.classList.remove("beVisible")
  ...
  hSelectList(event, data) {
    ...
    let dateMonth =
      1 +
      +document.querySelector(".calendarDate_month").getAttribute("data-month")
  ...
  createSection() {
    ...
    let elemSection = createElement("section", {
  ...
  createItem(task) {
    // createItem(data) если есть data = tasks from DB or state
    let className
    const currentULSection = document.querySelector(
      "section.active > .tabs_content_list"
    )
    if (currentULSection.querySelectorAll("input[type=text]").length > 18)
      return alert("Free version allows you create only 18 tasks")
    if (task != undefined) {
      className = {
    ...
    let elemCheckBox = createElement("input", {
      id: randomID,
      type: "checkbox",
      className: "checkbox",
      name: "check",
    ...
    let elemLILabel = createElement(
      "label",
      { className: className["label"] || "label" },
      task.item
    )
    let elemLIinput = createElement(
      "input",
      { type: "text", className: className["input"] },
      task.item
    )

    elemLI.innerHTML =
      '<div class="item_delete"><i class="fas fa-times"></i></div></li>'
  ...
  hdeleteItem(e) {
    ...
    const tabID = document
      .querySelector("div.tabs > div.active")
      .getAttribute("data-id")
    const labelsCollection = [
      ...document.querySelectorAll("section.active .item_delete i"),
    ]
    let indexItem = labelsCollection.indexOf(e.target) // get index of removing element
    this.emit(
      "deleteItem",
      { _id: tabID, index: indexItem, date: this.date },
      e
    )
  ...
  hupdateItem(e) {
    ...
    const inputCollection = [
      ...document.querySelectorAll("section.active input[type=text]"),
    ]
  ...
  hupdateCompState(label_, complete) {
    ...
    const tabID = document
      ...
      .getAttribute("data-id")
    const labelsCollection = [
      ...document.querySelectorAll("section.active label.label"),
    ]
  ...
  catch_focusOut(e) {
    if (e.type == "focusout") {
      ...
      if (
        (itemAmount <= 1 && e.target.value === "") ||
        e.target.value === prevLabelText
      )
        return
  ...
  addItem(event, id = null) {
    event.target.classList.add("input_hide")
  ...
  isTask(labelsCollection, del, day) {
    const labelsCollection1 = [
      ...document.querySelectorAll(
        "section.active > .tabs_content_list input[type=text]"
      ),
    ]
    let tdCollection = [...document.querySelectorAll("tbody td")]
  ...
  hAddItem(event) {
    ...
    if ((e.keyCode != 13) & (e.type != "focusout") & (e.target.tagName != "I"))
      return
  ...
  changeTab(event, cTab) {
    ...
    if (
      event.target.classList == "tab active" ||
      event.target.parentNode.classList == "tab active"
    )
      return
